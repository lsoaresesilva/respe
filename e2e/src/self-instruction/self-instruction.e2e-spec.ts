...
describe('Testes na página Self-instruction', () => {
    ...
    it("Deve apresentar uma mensagem de erro ao tentar avançar para edição sem preencher self-instruction", (done) => {
        browser.get("/").then(() => {
            browser.sleep(1000);
            ...
            page.navigateTo().then(() => {
                ...
                setTimeout(function () {
                    page.getResponderBtn().click().then(r => {
                        expect(page.getMensagemErro().isDisplayed()).toBeFalsy();
                        done();
                    })
                }, 2000)
