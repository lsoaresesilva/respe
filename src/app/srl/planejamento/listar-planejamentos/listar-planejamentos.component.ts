...
export class ListarPlanejamentosComponent implements OnInit {
    ...
    this.items = [
      {
        label: 'Alterar',
        icon: 'pi pi-check',
        command: (event) => this.alterar(this.selectedPlanejamento),
      },
      {
        label: 'Deletar',
        icon: 'pi pi-times',
        command: (event) => this.deletar(this.selectedPlanejamento),
      },
    ];
  ...
  exibirDialogPlanejamento() {
    if (
      localStorage.getItem('exibicaoDialogPlanejamento') == undefined ||
      localStorage.getItem('exibicaoDialogPlanejamento') != this.login.getUsuarioLogado().pk()
    ) {
  ...
  getPlanejamentos() {
    ...
    Planejamento.getAll(new Query('estudanteId', '==', usuario.pk())).subscribe(
      (planejamentosCadastrados) => {
  ...
  abrirPlanejamento(planejamento) {
    this.router.navigate([
      'main',
      { outlets: { principal: ['visualizacao-planejamento', planejamento.pk()] } },
    ]);
  }
  ...
  deletar(planejamento: Planejamento) {
    Planejamento.delete(planejamento.pk()).subscribe((resultado) => {
      Planejamento.getAll(new Query('estudanteId', '==', this.usuario.pk())).subscribe(
        (planejamentosCadastrados) => {
          this.planejamentos = planejamentosCadastrados;
        }
      );
  ...
  messageDeletar() {
    this.messageService.add({
      severity: 'error',
      summary: 'Deletado!',
      detail: 'Esse planejamento foi apagado!',
    });
  }
  ...
  alterar(planejamento: Planejamento) {
    if (planejamento != undefined) {
      this.router.navigate([
        'main',
        { outlets: { principal: ['atualizacao-planejamento', planejamento.pk()] } },
      ]);
    }
  }
}
