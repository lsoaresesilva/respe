...
export class VisualizarConteudoComponent implements OnInit {
  ...
  criarTagDestaque(tipo) {
    ...
    switch (tipo) {
      case TipoDestaque.highlight:
        ...
        tag = "<span id='destaque_"+index+"' style='background-color: #FFFF00; cursor:ponter' onclick='apagarHighlight(event, "+index.toString()+")'>$1</span>";
        break;
      case TipoDestaque.anotacao:
        tag = "<span id='destaque_"+index+"' style='background-color: rgba(204, 153, 255, 0.7);cursor:ponter' onclick=apagarAnotacao(event, "+index.toString()+")>$1</span>";
        break;
    }
  ...
  apagarHighlight(event, i) {
    ...
    this.panelApagarDestaque.show(event, document.getElementById("destaque_"+i));
    let _this = this;
