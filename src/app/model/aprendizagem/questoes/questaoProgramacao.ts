...
export class QuestaoProgramacao  extends QuestaoBase  {
  ...
  isRespostaCorreta(submissao: Submissao): boolean {
    ...
    for(let i = 0; i < submissao.resultadosTestsCases.length; i++){
      if (submissao.resultadosTestsCases[i] != null &&
        submissao.resultadosTestsCases[i].status !== true) {
  ...
  objectToDocument() {
    ...
    if (this.assuntos != null && this.assuntos.length > 0) {
      ...
      this.assuntos.forEach((assunto) => {
        if (assunto.pk != undefined && !assuntos.includes(assunto.pk())) {
    ...
    if (Array.isArray(this.testsCases) && this.testsCases.length > 0) {
      const ts = [];
  ...
  validar() {
    ...
    return super.validar() && validacao;
  }
